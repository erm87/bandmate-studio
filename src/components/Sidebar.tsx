  ...
  return (
    <aside className="flex w-80 shrink-0 flex-col overflow-y-auto border-r border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-950">
              ...
              onClick={(e) =>
                openMenu(e, [
                  ...
                  },
                  {
                    ...
                    onClick: () => setImportTrackMapOpen(true),
                  },
                ])
              }
            />
      ...
      <ImportTrackMapDialog
        ...
        onClose={() => setImportTrackMapOpen(false)}
      />
...
function SectionAction({
  ...
  return (
    <button
      ...
      // Tonal chip — same color family as the `tonal` Button variant
      // so adding a song / playlist / track-map reads as "the primary
      // workflow lives here." Visible at rest (was a near-invisible
      // ghost icon before), with the standard rest/hover/active
      // press-feedback states shared with the rest of the Button
      // system.
      className="flex h-6 w-6 items-center justify-center rounded-md bg-brand-50 text-brand-700 transition hover:bg-brand-100 active:bg-brand-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-400 focus-visible:ring-offset-2 dark:bg-brand-950/40 dark:text-brand-300 dark:hover:bg-brand-900/40 dark:active:bg-brand-800/50 dark:focus-visible:ring-offset-zinc-950"
    >
...
function PlusIcon({ className }: { className?: string }) {
  return (
    <svg
      ...
      fill="none"
      stroke="currentColor"
      // Bumped from 2 → 2.5 so the glyph reads with enough weight at
      // h-3.5 inside the new tonal SectionAction chip. Below ~2.25 the
      // plus disappears into the brand-50 background on hi-DPI displays.
      strokeWidth="2.5"
      strokeLinecap="round"
